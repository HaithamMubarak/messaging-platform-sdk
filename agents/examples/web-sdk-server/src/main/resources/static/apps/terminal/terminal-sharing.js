...
class TerminalSharing extends UserConnectionBase {
    constructor() {
        ...
        // File System message handlers (unified file system - local + SSH/SFTP)
        ...
        this.registerHandler('fs-navigate', (msg, src) => this.handleFileSystemNavigate(msg, src));
        this.registerHandler('fs-notification', (msg, src) => this.handleFileSystemNotification(msg, src));
    ...
    // ========================================
    // FILE SYSTEM NAVIGATION SHARING
    // ========================================

    /**
     * Share file system navigation state with all viewers
     * Called when owner navigates to a different directory
     * Works for both local and SSH/SFTP file systems
     */
    shareFileSystemNavigation(sessionId, path, files) {
        console.log(`[TerminalSharing] Sharing file system navigation for session ${sessionId}:`, path);

        // Broadcast to all connected agents using unified fs-navigate type
        this.sendData({
            type: 'fs-navigate',  // ✅ Use unified fs-navigate instead of sftp-navigate
            sessionId: sessionId,
            path: path,
            files: files,
            timestamp: Date.now()
    ...
    // ========================================
    // FILE SYSTEM SHARING (Unified - Local + SSH/SFTP)
    // ========================================
