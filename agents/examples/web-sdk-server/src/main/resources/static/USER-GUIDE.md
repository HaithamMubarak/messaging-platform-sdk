...
1. **Client** creates AgentConnection with API key
2. **Agent** connects to messaging service via WebSocket
3. **Service** publishes messages to Kafka topics
4. **Agent** receives messages in real-time

### Components

- **AgentConnection** - Main client class for messaging
- **Channel** - Named communication space (like a room)
- **Message** - Data packet with offset tracking
- **WebRTC Helper** - Audio/video streaming support

### Agent Name Requirements

**IMPORTANT:** Agent names must follow these rules:

- ✅ **Allowed characters:** Letters (a-z, A-Z), numbers (0-9), hyphens (-), underscores (_)
- ❌ **Not allowed:** Spaces, special characters (@, #, $, %, etc.)
- ✅ **Examples:**
  - Valid: `user-1`, `agent_123`, `WebRTC-Relay-1`, `bot_assistant`
  - Invalid: `user@1`, `agent #123`, `my agent`, `bot$special`

**Validation:**
The server validates agent names and will reject invalid names with an error:
```
Invalid agentName: must contain only letters, numbers, hyphens (-), and underscores (_)
```
...
```gradle
repositories {
    maven { url '../messaging-platform-sdk/libs' }
}

dependencies {
    implementation 'com.hmdev.messaging:java-agent:1.0.0'
    implementation 'com.hmdev.messaging:messaging-common:1.0.0'
}
```
...
public class MessagingExample {
    public static void main(String[] args) throws Exception {
        // 1. Create connection
        AgentConnection agent = new AgentConnection(
            "http://localhost:8082",  // API URL
            "your-api-key"             // API key
        );
        
        // 2. Set message listener
        agent.setOnMessage(message -> {
        ...
        agent.connect(
            ...
            "java-agent-1",    // Agent name
            "secret123"        // Channel password
        );
...
4. **Check online documentation:**
   - Visit https://hmdevonline.com/messaging-platform/web-agent/docs.html
   - Browse live examples at https://hmdevonline.com/messaging-platform/web-agent/
...
- **Web Agent:** Complete JavaScript SDK with WebRTC support
- **Java Agent:** Desktop and server applications
- **Python Agent:** Scripts, bots, and automation
- **C++ Agent:** High-performance game integration

### Example Applications

```
messaging-platform-sdk/agents/examples/
├── web-demos-server/src/main/resources/static/examples/
│   ├── chat.html - Full-featured web chat
│   ├── webrtc.html - Video/audio streaming
│   ├── whiteboard/ - Collaborative drawing
│   ├── quickshare/ - P2P file sharing
│   └── mini-games/ - Multiplayer game examples
```

### Getting Help

- **Documentation:** Visit https://hmdevonline.com/messaging-platform/web-agent/docs.html
- **Developer Portal:** Manage API keys and monitor usage
- **GitHub:** https://github.com/HaithamMubarak/messaging-platform-sdk
- **Live Demos:** https://hmdevonline.com/messaging-platform/web-agent/
...
repositories {
    maven { url 'https://repo.hmdevonline.com/maven' }
}
dependencies {
    implementation 'com.hmdev.messaging:java-agent:1.0.0'
}
...
This SDK is provided for use with the Messaging Platform services.

---

**Need API Access?**  
Visit the [Developer Portal](https://hmdevonline.com/messaging-platform/web-agent/#plans) to request an API key.

**Questions?**  
Check the troubleshooting section above or explore our [interactive demos](https://hmdevonline.com/messaging-platform/web-agent/).
